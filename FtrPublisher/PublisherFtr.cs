...
namespace FtrPublisher
{
    ...
    public class Publisher : IProcessor  
    {
        ...
        private string _uri;
        private Dictionary<string, Restrictions> _clientes = new Dictionary<string, Restrictions>();
        ...
        public bool proccess(object payload, object metadata = null)
        {
            try
            {
                ...
                mut.WaitOne();
                foreach (KeyValuePair<string , Restrictions> entry in _clientes)
                {
                    if (entry.Value.IsAutorized())//toDo: implemetar el mecanismo para validar autorizacion a recibir el mensaje por cada cliente conectado
                    {
                        Client client = server.GetConnectedClient(entry.Key);
                        if (client is null)
                        {
                            _logger.Error($"No se encontro la conexion con el cliente conectado al socket '{client.GetSocket().ToString()}' ");
                        }
        ...
        private void initEvent()
        {
            ...
            server.OnClientDisconnected += (object sender, OnClientDisconnectedHandler e) =>
            {
                if (forceDisconetClient is false)
                {
                    ...
                    this._clientes.Remove(e.GetClient().GetGuid());
            ...
            server.OnMessageReceived += (object sender, OnMessageReceivedHandler e) =>
            {
                ...
                if (this.IsTotpValid(bytes, sTotp))
                {
                    ...
                    Restrictions clientRestriction = new Restrictions();
                    this._clientes.Add(e.GetClient().GetGuid(), clientRestriction);
